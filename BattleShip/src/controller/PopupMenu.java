...
class NavyUpListener implements ActionListener {
	public void showRemaining() {
		...
		Controller.getTheOnlyInstance().getTheView().getFramePlayer()
				.displayErrorMessage("there are left " + field.getCountNavy() + " Navies");
	}

	public void actionPerformed(ActionEvent arg0) {
		try {
			FrameOpponent opponentFrame = Controller.getTheOnlyInstance().getTheView().getFrameOpponent();
			Point point = new Point(ProcessingPlayer.getPosX(), ProcessingPlayer.getPosY());
			Field field = Controller.getTheOnlyInstance().getTheModel().getOne().getField();
			ProcessingPlayer sketch = Controller.getTheOnlyInstance().getTheView().getFramePlayer().getSketch();
